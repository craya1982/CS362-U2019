...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  case minion:
    return minionCard(handPos, currentPlayer, choice1, state);
  ...
  case tribute:
    return tributeCard(state, nextPlayer, currentPlayer);
  case ambassador:
    return ambassadorCard(choice2, choice1, currentPlayer, handPos, state);
...
/**
 * Handles a baron card being played
 */
  ...
  {                             //Boolean true or going to discard an estate
    ...
    while (card_not_discarded)
    {
      ...
      else if (p > state->handCount[currentPlayer])
      {
        ...
        handleBaronGainEstate(state, currentPlayer);
        card_not_discarded = 0; //Exit the loop
      }
...
/**
 * Helper function that handles the case when a player should gain an
 * estate after playing the baron card
 */
void handleBaronGainEstate(struct gameState *state, int currentPlayer)
{
...
/**
 * Helper method for when playing a minion card
 */
int minionCard(int handPos, int currentPlayer, int chooseCoinsFlag, struct gameState *state)
{
  //+1 action
  int i, j;
  state->numActions++;
...
/**
 * Helper method for when playing an ambassador card
 */
int ambassadorCard(int amountOfCardsToDiscard, int cardToDiscardPosition, int currentPlayer, int handPos, struct gameState *state)
{
  ...
  for (i = 0; i < state->handCount[currentPlayer]; i++)
  {
    if (i != handPos && i == state->hand[currentPlayer][cardToDiscardPosition] && i != cardToDiscardPosition)
    {
      j++;
    }
  }
  if (j < amountOfCardsToDiscard)
  {
    return -1;
  }

  if (DEBUG)
    printf("Player %d reveals card number: %d\n", currentPlayer, state->hand[currentPlayer][cardToDiscardPosition]);
  ...
  for (i = 0; i < state->numPlayers; i++)
  {
    if (i != currentPlayer)
    {
      gainCard(state->hand[currentPlayer][cardToDiscardPosition], state, 0, i);
    }
  }
  ...
  for (j = 0; j < amountOfCardsToDiscard; j++)
  {
    for (i = 0; i < state->handCount[currentPlayer]; i++)
    {
      if (state->hand[currentPlayer][i] == state->hand[currentPlayer][cardToDiscardPosition])
      {
...
/**
 * Helper method for when playing a tribute card
 */
...
/**
 * Helper method for when playing a mine card
 */
int mineCard(int currentPlayer, struct gameState *state, int cardToTrash, int selectedTreasure, int handPos)
{
  ...
  gainCard(selectedTreasure, state, 2, currentPlayer);
